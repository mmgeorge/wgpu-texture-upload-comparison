...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
  ...
  let stagingBuffers: GPUBuffer[] = [];

  function getOrCreateStagingBuffer(): GPUBuffer {
  ...
  function renderWithWriteTexture() {
    (passDesc as any).colorAttachments[0].view = context.getCurrentTexture().createView()
  ...
  function renderWithMappedBufferToTexture() {
    ...
    encoder.copyBufferToTexture(
      { buffer: staging, bytesPerRow: 4096 * 4 }, 
      { texture }, {
      width: 4096, 
      height: 4096, 
    })
